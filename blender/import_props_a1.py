...
if fourCC != 0:
    raise NotImplementedError("Can't decompress gamelumps just yet, use:\nbspzip -repack <bspfile> and try again")
file.seek(offset)
...
fourCC = int.from_bytes(file.read(4), "little")
if fourCC != 0:
    raise NotImplementedError("Can't decompress entitiy lumps just yet")
file.seek(offset)
...
for entity in entities:
    if 'classname" "prop_' in entity:
        ...
        entity = list(filter(lambda x: x != "", entity))  # multiple per entity
        entity_struct = {}
    ...
    try:
        ...
        if len(decompiled_smds) == 1:
            ...
            for smd in decompiled_smds:
                if smd.endswith(("_reference.smd", "model.smd")) or smd.lower() == filename + ".smd":
                    # ^ can replace with lod1 or similar
